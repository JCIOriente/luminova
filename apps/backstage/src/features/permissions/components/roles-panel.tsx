...
import { Badge, Button, Card, EmptyState, Sheet } from "@luminova/ui";
...
/** The single `/permisos` role list: one row per role doc, showing what confers it, who
 *  holds it, and its permission count — plus the create/edit/delete editor. Replaces the
 *  old split between PermisosView (cargo-derived) and RoleManager (role docs), which
 *  rendered the same role under two different names on one screen. */
    ...
    <section className="flex flex-col gap-4">
      ...
      {rows.length === 0 ? (
        <EmptyState
          ...
          description="Ejecuta la sincronización de roles para crear los predefinidos."
        />
      ) : (
        <Card as="ul" padding="none" className="flex flex-col divide-y divide-line">
          {rows.map((row) => (
            <li key={row.role.id} className="flex flex-col gap-3 px-4 py-3">
              <div className="flex items-center justify-between gap-3">
                <div className="flex flex-col gap-0.5">
                  <div className="flex items-center gap-2">
                    <span className="font-medium text-ink-1">{row.role.name}</span>
                    <Badge tone={row.role.builtIn ? "navy" : "teal"}>
                      {row.role.builtIn ? "Predefinido" : "Personalizado"}
                    </Badge>
                    {row.role.locked && <Badge tone="gray">Protegido</Badge>}
                  </div>
                  {row.role.description && (
                    <span className="text-ui-sm text-ink-3">{row.role.description}</span>
                  )}
                  <span className="text-ui-xs text-ink-3">
                    {row.role.permissions.length} permiso
                    {row.role.permissions.length === 1 ? "" : "s"}
                  </span>
                </div>
                <Button
                  as="button"
                  variant="secondary"
                  size="sm"
                  onClick={() => setEditing(row.role)}
                >
                  {row.role.locked ? "Ver" : "Editar"}
                </Button>
              </div>
