...
customs) and read only by Admins. `role-repository.test.ts:47` pins the absence — both
`where` and `query` must go uncalled — so re-narrowing it goes red loudly. Full rationale in
