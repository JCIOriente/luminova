...
| G4 🟡 | App Check enforcement ON (after reCAPTCHA keys) — **client code scaffolded** (`@luminova/firebase` inits reCAPTCHA v3 App Check when `VITE_APPCHECK_SITE_KEY` set; debug-token supported). Remaining = infra: provision key, set env, flip enforcement. **`redeemInvite` and `describeInvite` are the FIRST functions to flip** — the project's only unauthenticated callables, declared `enforceAppCheck: false`. NB the site key already exists in `.env.production` and clients already send a token; the remaining blocker for these two is that enforcement is per-PRODUCT and only Firestore + Storage are confirmed enabled. Flip deliberately with the invite page tested: they then hard-depend on `VITE_APPCHECK_SITE_KEY` in the backstage build, so a misconfigured deploy silently breaks all member onboarding. See `docs/firebase-setup.md`. | infra keys | `[S]` | `/security-review` |
