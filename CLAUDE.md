...
- **Recurring pitfalls (2026-07 audit) — guardrails.** The mistake-classes that each recurred across multiple audit findings, now standing rules. Detail + real examples + the enforcing guard for each in `docs/engineering-guardrails.md`.
  1. **Extract, don't copy.** Same logic in 2+ places → parameterize/extract (rule of three), consolidate-when-touched — never copy-paste a feature/hook/shell. UI duplication specifically: `docs/reuse-first-ui.md`.
...
- `docs/engineering-guardrails.md` — the 7 recurring mistake-classes from the 2026-07 audit, each with the rule + a real example + the enforcing guard (see the "Recurring pitfalls" rules under Cross-Cutting Discipline)
